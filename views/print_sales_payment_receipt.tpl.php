...
<section class="sheet padding-10mm no-height">
    <div id="topButtons" style="position: absolute;top: 5mm;right: 5mm;">
        <?if($_GET['redirect']){?>
        <a type="button" href="<?= base64_decode($_GET['redirect']) ?>" class="btn btn-success">Back</a>
        <?}?>
        ...
        <table class="table table-bordered">
            ...
            <tr>
                <td colspan="2">
                    ...
                    <p style="margin: 1mm">Payment Received: <span
                                class="value"><?= formatN($paymentInfo['received_amount']) ?></span></p>
                    <p style="margin: 1mm">Amount in words: <span class="value"
                                                                  style="text-transform: capitalize"><?= toWords($paymentInfo['received_amount']) ?></span>
                    </p>
                </td>
            </tr>
            <tr>
                <td colspan="2">
                    <? if ($paymentInfo['advance_amount'] > 0) { ?>
                        <p style="margin: 1mm"> Offset Amount: <span
                                    class="value"><?= $paymentInfo['currencyname'] ?> <?= formatN($paymentInfo['advance_amount']) ?></span></p>
                    <? } ?>
            ...
            <tr>
                <td colspan="2">
                    <div class="d-flex justify-content-center">
                        <table class="table table-condensed inner-table" style="width: 50%;margin:0;">
                            <thead>
                            <tr>
                                <th>Invoice No</th>
                            ...
                            <? foreach ($paymentInfo['payments'] as $index => $payment) { ?>
                                <tr>
                                    ...
                                    <td align="right"><?= formatN($payment['amount']) ?> <?= $paymentInfo['currencyname'] ?></td>
                                </tr>
                            <? } ?>
                            </tbody>
                        </table>
