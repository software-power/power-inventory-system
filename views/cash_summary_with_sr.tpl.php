...
<div class="row d-flex justify-content-center">
    <div class="col-xl-8 col-md-10">
        <section class="panel">
            <header class="panel-heading"><div class="row">
                    <div class="col-md-6">
                        <h2 class="panel-title">Branch Cash Summary</h2>
                    </div>
            ...
            <div class="panel-body">
                <div class="table-responsive">
                    <table class="table table-hover mb-none table-bordered" style="font-size:13px;">
                        ...
                        <tr>
                            <td>SR Sales Receipts</td>
                            <? foreach ($usedCurrency as $c) { ?>
                                <td class="text-right"><?= formatN($totalSrCashSalesPayment[$c]) ?></td>
                            <? } ?>
                        </tr>
                        <tr>
                            <td>Credit Notes</td>
                            <?foreach ($usedCurrency as $c){?>
                                <td class="text-right text-rosepink"><?= formatN($totalSalesReturn[$c]) ?></td>
                            <?}?>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td align="right" class="text-danger">
                                <strong><?= $baseCurrency['name'] ?> <?= formatN($totalExpenses[$baseCurrency['name']]) ?></strong></td>
                            <td></td>
                        </tr>
                        <tr>
                            <td colspan="<?= 1 + count($usedCurrency) ?>" class="pb-lg"></td>
...
<script>
    function fetchSummary() {
        ...
        let date = $('#date').val();
        window.location.replace(`?module=reports&action=cash_summary_with_sr&branchid=${branchid}&date=${date}`)
    }
</script>
