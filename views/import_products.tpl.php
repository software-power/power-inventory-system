    ...
    <div class="col-md-4">
        <section class="panel">
            <header class="panel-heading d-flex justify-content-between">
                ...
                <? } else { ?>
                    <h2 class="panel-title">Import Products and Masters</h2>
                    <a href="<?= url('import', 'download_product_template') ?>" class="btn btn-primary btn-sm">Download template</a>
                <? } ?>
            </header>
            <div class="panel-body">
                <? if (isset($upload_result)) { ?>
                    <? foreach ($upload_result as $item => $R) { ?>
                        <h5 class="text-weight-bold"><?= ucfirst($item) ?></h5>
                        <ul>
                            <? foreach ($R as $action => $count) { ?>
                    ...
                    <form action="<?= url('import', 'upload_products') ?>" method="post" enctype="multipart/form-data"
