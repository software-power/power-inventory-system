...
<header class="page-header">
    <h2>GRN Supplier Payment</h2>
</header>
<div class="center-panel">
    <section class="panel">
        <header class="panel-heading for-heading">
            <div class="d-flex justify-content-between">
                <h2 class="panel-title"><i class="fa fa-money"></i> GRN Supplier Payments </h2>
                <div class="d-flex">
        ...
        <div class="panel-body">
            <fieldset class="row-panel">
                <legend>GRN Info</legend>
                <div class="row">
                    <div class="col-md-6">
                        <p>GRN No: <span class="text-primary"><?= $grn['id'] ?></span></p>
                        <p>Invoice No: <span class="text-primary"><?= $grn['invoiceno'] ?></span></p>
                        <p>Supplier Name: <span class="text-primary"><?= $grn['suppliername'] ?></span></p>
                        <p>Payment Status:
                            <? if ($grn['payment_status'] == PAYMENT_STATUS_COMPLETE) { ?>
                                <span class="text-success text-uppercase" style="font-weight: bold">Completed</span>
                            <? } elseif ($grn['payment_status'] == PAYMENT_STATUS_PARTIAL) { ?>
                        ...
                        <p>Currency: <span class="text-primary"><?= $grn['currency_name'] ?></span>
                        <p>Base Exchange Rate: <span class="text-primary"><?= $grn['currency_amount'] ?></span>
                        <p>Full Amount: <span class="text-primary"><?= formatN($grn['full_amount']) ?></span>
                        </p>
                        <p>Total Paid: <span
                                    class="client_tin text-success"><?= formatN($grn['total_paid']) ?></span>
                        </p>
                        <p>Pending Amount: <span
                                    class="text-danger"><?= formatN($grn['outstanding_amount']) ?></span>
                        </p>
                    </div>
                    <div class="col-md-6">
                        <p>LPO: <span class="text-primary"><?= $grn['lpoid'] ? $grn['lpoid'] : '' ?></span></p>
                        <p>Payment Type: <span class="text-primary"><?= ucfirst($grn['paymenttype']) ?></span></p>
                        <p>Supplier Payment: <span class="text-primary"><?= ucfirst($grn['supplier_payment'] ? 'Yes' : 'No') ?></span></p>
                        <p>Issue Date: <span class="client_address text-primary"><?= fDate($grn['doc']) ?></span></p>
                        <p>Created by: <span class="client_tin text-primary"><?= $grn['creator'] ?></span></p>
                    </div>
                </div>
            </fieldset>
            <div class="table-responsive mt-xlg">
                <table class="table table-hover mb-none" id="amcTable" style="font-size:13px">
                    ...
                    <tr>
                        ...
                        <th style="text-align:center" title="Invoice Number"><i></i> Currency</th>
                        <th style="text-align:center" title="Invoice Number"><i></i> Base Exchange Rate</th>
                    ...
                    <? } else { ?>
                        <? foreach ($payments as $id => $R) { ?>
                            <tr>
                                ...
                                <td style="text-align:center;text-transform:uppercase"><?= $R['currency_name'] ?></td>
                                <td style="text-align:center;text-transform:uppercase"><?= $R['currency_amount'] ?></td>
