...
<header class="page-header">
    <h2>Stock Report With Supplier</h2>
</header>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form>
                ...
                <input type="hidden" name="action" value="stock_report_admin_with_supplier">
    ...
    <section class="panel">
        <header class="panel-heading">
            <div class="panelControl">
                <button class="btn" href="#search-modal" data-toggle="modal"><i class="fa fa-search"></i> Open Search</button>
                <a class="btn" href="?module=home&action=index" title="Home"> <i class="fa fa-home"></i> Home</a>
            </div>
            <h2 class="panel-title">Stock Report With Last 2 Supplier</h2>
                ...
                <table class="table table-hover mb-none" style="font-size:13px;" id="userTable">
                    ...
                    foreach ($stocklist as $ins => $list) { ?>
                        <tr>
                            ...
                            <td><?= $list['total'] ?></td>
                            <td>
                                <? foreach ($list['suppliers'] as $purchase) { ?>
                                    <div>
                                        GRN <?= $purchase['grnid'] ?>, <?= $purchase['suppliername'] ?>, Qty: <?= $purchase['qty'] ?>,Bill Qty: <?= $purchase['billable_qty'] ?>, Rate: <?= formatN($purchase['rate']) ?>, <?= fDate($purchase['doc']) ?>
                                    </div>
                                <? } ?>
                            </td>
