<div class="modal fade" id="sales-document-modal" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="sales-document-modal"
     aria-hidden="true">
    <div class="modal-dialog modal-dialog-center">
        <div class="modal-content">
            ...
            <form action="<?= url('sales', 'attach_document') ?>" method="post" enctype="multipart/form-data">
                <input type="hidden" name="salesid" class="salesid">
...
<script>
    let sales_document_modal = $('#sales-document-modal');

    $(sales_document_modal).on('show.bs.modal', function (e) {
        let source = $(e.relatedTarget);
        let modal = $(this);
        let salesid = source.data('saleid');
        let approved = source.data('approved') == 1;
        console.log('approved: ',approved);
        $(modal).find('.salesid').val('').val(salesid);
        $(modal).find('.invoiceno').text('').text(source.data('invoiceno'));


        $(sales_document_modal).find('tbody').empty();
        $.get(`?module=sales&action=getSalesDocument&format=json&salesid=${salesid}`, null, function (data) {
            $(sales_document_modal).find('tbody').empty();
            ...
            if (result.status === 'success') {
                $.each(result.data, function (i, item) {
                    ...
                    $(sales_document_modal).find('tbody').append(row);
                });
            } else {
